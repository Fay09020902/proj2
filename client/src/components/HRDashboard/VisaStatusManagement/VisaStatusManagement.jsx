...
const getNextStep = (emp) => {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/hr/visa-status/all', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const optInProgress = employees.filter(
    emp =>
      emp.submittedAllOPT === false &&
      emp.workAuth === 'F1(CPT/OPT)'
  );
  ...
  return loading ? (
    ...
    <Tabs defaultActiveKey="1" destroyInactiveTabPane>
      <TabPane tab="OPT Visa In Progress" key="1">
        <VisaStatusForm employees={optInProgress}
  mode="in-progress"
  ...
  getDaysRemaining={getDaysRemaining}/>
      ...
      <TabPane tab="All Visa Status" key="2">
        <VisaStatusForm employees={employees}
        mode="all"
        handleDownload={handleDownload}
  handlePreview={handlePreview}
  getNextStep={getNextStep}
